...
function AnimeCard({ anime }: AnimeCardProps) {
  ...
  const title = (anime.title || (anime as any).name || (anime as any).titles?.[0]?.title || "Untitled") as string;

  const titleForSlug = (((anime as any)?.title_english || title || "anime") as string).trim();
  const slug = React.useMemo(() => slugify(titleForSlug), [titleForSlug]);

  const { t } = useTranslation();

  // ✅ tu hook devuelve string, no { pastelColor }
  const pastelColor = useStablePastelColor(anime.mal_id);

  // Ojo: ahora el ref apunta al <a> del Link
  const cardRef = React.useRef<HTMLAnchorElement>(null);

  const [isButtonVisible, setIsButtonVisible] = React.useState(false);
  const [isFavorited, setIsFavorited] = React.useState(false);

  const { placement, handleMouseEnter, handleMouseLeave } = useCardTooltip(cardRef as any);
  const { seasonInfo, scoreColor, typeInfo } = useAnimeCardData(anime as any);
  ...
  // ✅ IMPORTANTE: si está dentro de <Link>, stopPropagation NO basta.
  // Hay que hacer preventDefault para que el click del corazón NO navegue.
            ...
            <div className="flex flex-wrap gap-1">
              {anime.genres.slice(0, 3).map((genre) => (
                <span
                  ...
                  style={{
                    backgroundColor: `${pastelColor ? String(pastelColor) : "#a855f7"}30`,
                    color: pastelColor ? String(pastelColor) : "#a855f7",
                  }}
                >
                  {genre.name}
                </span>
